...
public class SortedBinaryTree<T extends Comparable<T>> implements Tree<T> {
    ...
    protected int calcHeight(TreeNode<T> node) {
        int height = 0;
        if (node != null) {
            height = calcHeightRekursiv(node);
        }
        return height;
    }

    private int calcHeightRekursiv(TreeNode<T> node) {
        if (node != null) {
            int heightLeft = calcHeightRekursiv(node.left);
            int heightRight = calcHeightRekursiv(node.right);
    ...
    protected int calcSize(TreeNode<T> node) {
        int size = 0;
        if (node != null) {
            size = calcSizeRekursiv(node, size);
        }
        return size;
    }
    private int calcSizeRekursiv(TreeNode<T> node, int size) {
        if (node != null) {
            size ++;
            if (node.left != null) { size = calcSizeRekursiv(node.left, size);}
            if (node.right != null) { size = calcSizeRekursiv(node.right, size);}
        }
        return size;
    }
